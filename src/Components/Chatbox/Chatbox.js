...
const Chatbox = () => {
 ...
 const realTimeAccountListener = onSnapshot(accountsQuery, (snapshot) => {
 ...
 const chatUsersFromFireBaseWithoutLoggedInUser = newChatUsers.filter(
 ...
 let singleUser = {
 ...user,
 notificationValue: getNotificationValue(user.email),
 };
 return singleUser;
 });

 allUsers = sortBy(allUsers, "notificationValue");
 setChatUsers(allUsers.reverse());
