...
const Explore = () => {
  ...
  const incidentType = ["audio", "image", "text", "video", "All"];
  ...
  const getIncidentsByMonthAndYear = (month, year, type, category) => {
    ...
    const filtered_incidents = incidents.filter((incident) => {
      if (
        incident.properties.categories != undefined &&
        incident.properties.categories.length != 0
      ) {
        return (
          incident.properties.month === month &&
          incident.properties.year === year &&
          incident.properties.file.type === type &&
          incident.properties.categories.indexOf(category) !== -1
        );
      }
    });
  ...
  const getIncidentsByAllType = (month, year) => {
    ...
    const filtered_incidents = incidents.filter((incident) => {
      return (
        incident.properties.month === month && incident.properties.year === year
      );
    });
  ...
  useEffect(() => {
    const getIncidentsDataFromFireStore = async (db) => {
      ...
      const incidentsListData = querySnapshot.docs.map((doc) => {
        ...
        if (data.properties.categories != undefined) {
    ...
    map.current.on("load", () => {
      ...
      const updateMarkerForSinglePoint = () => {
        ...
        for (const feature of features) {
          ...
          let marker = new mapboxgl.Marker({ element: el }).setLngLat(coords);
      ...
      const updateMarkers = () => {
        ...
        for (const feature of features) {
          ...
          map.current
            .getSource("incidents")
            .getClusterLeaves(id, pointCount, 0, (error, cluster_features) => {
              // if no error then store the data of all points in the cluster_points_data array
              if (!error) {
                cluster_points_data =
                  // iterate through the cluster_features array
                  cluster_features.map((cluster_point) => {
                    return {
                      properties: cluster_point.properties,
                      geometry: cluster_point.geometry,
                    };
                  });
              }
            });
